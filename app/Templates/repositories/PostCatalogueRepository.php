...
class {$class}CatalogueRepository extends BaseRepository implements {$class}CatalogueRepositoryInterface
{
    ...
    public function get{$class}CatalogueById(int $id = 0,$language_id = 0){
        return $this->model->select(['{module}_catalogues.id',
                                     ...
                                     'tb2.canonical'
        ])->join('{module}_catalogue_language as tb2','tb2.{module}_catalogue_id', '=', '{module}_catalogues.id')->where('tb2.language_id', '=', $language_id)->find($id);
    }
