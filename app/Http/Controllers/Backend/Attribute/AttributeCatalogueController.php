...
class AttributeCatalogueController extends Controller
{
    ...
    public function store(StoreAttributeCatalogueRequest $request){
        ...
        return redirect()->route('attribute.catalogue.index')->with('error','Thêm mới bản ghi thất bại. Hãy thử lại');    
    }

    public function edit($id){
        $this->authorize('modules', 'attribute.catalogue.update');
        $attributeCatalogues = $this->attributeCatalogueRepository->getAttributeCatalogueById($id,$this->language);
        $config = $this->configData();
        $config['seo'] = __('messages.attributeCatalogue');
        $config['method'] = 'edit';
        $dropdown = $this->nestedset->Dropdown();
    ...
    public function update($id,UpdateAttributeCatalogueRequest $request){
        ...
        return redirect()->route('attribute.catalogue.index')->with('error','Cập nhật bản ghi thất bại. Hãy thử lại');    
    }
    ...
    public function destroy($id){
        ...
        return redirect()->route('attribute.catalogue.index')->with('error','Xóa bản ghi thất bại. Hãy thử lại');  
    }
