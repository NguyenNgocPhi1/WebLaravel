        ...
        <div class="uk-flex uf-flex-middle uk-flex-space-between">
            ...
            <div class="action">
                <div class="uk-flex uk-flex-middle">
                    ...
                    @endphp
                    <select name="product_catalogue_id" class="form-control mr10 setupSelect2">
                        @foreach ($dropdown as $key => $val)
                        <option {{ ($productCatalogueId==$key) ? 'selected' : '' }} value="{{ $key }}">{{ $val }}
                        </option>
                        @endforeach
                    </select>
                    @include('backend.dashboard.component.keyword')
                    <a href="{{ route('product.create') }}" class="btn btn-danger"><i class="fa fa-plus mr5"></i>{{
                        __('messages.product.create.title') }}</a>
                </div>
            </div>
        </div>
    </div>
