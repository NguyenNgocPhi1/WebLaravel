(function($){
    ...
    HT.chooseApplyItem = () => {
        $(document).on('change', '.conditionItem', function(){
            ...
            for (let i = 0; i < condition.value.length; i++) {
                ...
                let label = condition.label[i].text
                let html = HT.createConditionItem(value, label)
            }
        })
    }
    ...
    HT.createConditionLabel = (label, value) => {
        ...
        wrapperBox.append(flex).prop('outerHTML')
        return wrapperBox
        ...
        if(!$('.wrapper-condition').find('.' + value).elExist()){
            $.ajax({
                ...
                success: function(res) {
                    ...
                    conditionItem.append(select).prop('outerHTML')
        ...
        let html = `
            <div class="product-and-quantity">
                <div class="choose-module mt20">
                    <div style="color: blue" class="fix-label" for="">Sản phẩm áp dụng</div>
                            ...
                            <td class="discountType">
                                <div class="uk-flex uk-flex-middle">
                                    ...
                                    <select name="product_and_quantity[discountType]" class="multipleSelect2" id="">
                                        <option ${ (preloadData.discountType == 'cash') ? 'selected' : '' } value="cash">đ</option>
                                        <option ${ (preloadData.discountType == 'percent') ? 'selected' : '' } value="percent">%</option>
                                    </select>
                                </div>
                            </td>
                        </tr>
    ...
    HT.productQuantityFindProduct = () => {
        $(document).on('click', '.product-variant', function(e){
            e.preventDefault()
            let option = {
                model: $('.select-product-and-quantity').val()
            }
    ...
    HT.fillProductCatalogueToList = (object) => {
        ...
        if(object.data.length){
            ...
            for (let i = 0; i < object.data.length; i++) {
                ...
                let isChecked = ($(`.boxWrapper .${classBox}`).length) ? 'checked' : ''
                html += `
                    <div class="search-object-item" data-product_id="${id}" data-name="${name}">
    ...
    HT.fillProductToList = (object) => {
        ...
        if(object.data.length){
            ...
            for (let i = 0; i < object.data.length; i++) {
                ...
                html += `
                    <div class="search-object-item" data-product_id="${product_id}" data-variant_id="${product_variant_id}" data-name="${name}" data-uuid="${uuid}">
                        <div class="uk-flex uk-flex-middle uk-flex-space-between">
                            <div class="object-info">
                                <div class="uk-flex uk-flex-middle">
                                    <input 
