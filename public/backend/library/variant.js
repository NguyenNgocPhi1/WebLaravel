(function ($) {
    ...
    HT.renderVariantItem = (attributeCatalogue) => {
        ...
        html = html + '<div class="row mb20 variant-item">'
            ...
            html = html + '<div class="col-lg-1">'
                html = html + '<button type="button" class="remove-attribute btn btn-danger">'
                    html = html + '<svg data-icon="TrashSolidLarge" aria-hidden="true" focusable="false" width="15" height="16"'
                        html = html + 'viewBox="0 0 15 16" class="bem-Svg" style="display: block">'
                        html = html + '<path fill="currentColor"'
                            html = html + 'd="M2 14a1 1 0 001 1h9a1 1 0 001-1V6H2v8zM13 2h-3a1 1 0 01-1-1H6a1 1 0 01-1 1H1v2h13V2h-1z">'
                        html = html + '</path>'
                    html = html + '</svg>'
                html = html + '</button>'
            html = html + '</div>'
    ...
    HT.createVariantRow = (attributeItem, variantItem) => {
        ...
        $td = $('<td>').append(
            $('<span>').addClass('image img-cover').append(
                $('<img>').attr('src', 'https://images.spiderum.com/sp-images/b999ab407d6811ed8117892817d55697.jpeg').addClass('imageSrc')
            )
        )
        $row.append($td)
            ...
            for(let i = 0; i < album.length; i++){
                html = html + '<li class="ui-state-default">'
                    html = html + '<div class="thumb">'
                        ...
                        html = html + '</span>'
                        html = html + '<button class="variant-delete-image"><i class="fa fa-trash"></i></button>'
                    html = html + '</div>'
                html = html + '</li>'
            }
        }
                        ...
                        html = html + '<div class="uk-flex uk-flex-middle uk-flex-space-between">'
                            ...
                            html = html + '<div class="button-group">'
                                html = html + '<div class="uk-flex uk-flex-middle">'
                                    html = html + '<button type="button" class="cancleUpdateVariant btn btn-danger mr10">Hủy bỏ</button>'
                    ...
                    html = html + '<div class="ibox-content">'
                        html = html + '<div class="click-to-upload-variant '+( (variantAlbum.length > 0 && variantAlbum[0] !== '') ? 'hidden' : '' )+'">'
                            html = html + '<div class="icon">'
                                html = html + '<a href="" type="button" class="upload-variant-picture">'
                                    html = html + '<svg style="width:80px;height:80px;fill: #d3dbe2;margin-bottom: 10px;"'
                                        html = html + 'xmlns="http://www.w3.org/2000/svg" viewBox="0 0 80 80">'
                                        html = html + '<path '
                                            html = html + 'd="M80 57.6l-4-18.7v-23.9c0-1.1-.9-2-2-2h-3.5l-1.1-5.4c-.3-1.1-1.4-1.8-2.4-1.6l-32.6 7h-27.4c-1.1 0-2 .9-2 2v4.3l-3.4.7c-1.1.2-1.8 1.3-1.5 2.4l5 23.4v20.2c0 1.1.9 2 2 2h2.7l.9 4.4c.2.9 1 1.6 2 1.6h.4l27.9-6h33c1.1 0 2-.9 2-2v-5.5l2.4-.5c1.1-.2 1.8-1.3 1.6-2.4zm-75-21.5l-3-14.1 3-.6v14.7zm62.4-28.1l1.1 5h-24.5l23.4-5zm-54.8 64l-.8-4h19.6l-18.8 4zm37.7-6h-43.3v-51h67v51h-23.7zm25.7-7.5v-9.9l2 9.4-2 .5zm-52-21.5c-2.8 0-5-2.2-5-5s2.2-5 5-5 5 2.2 5 5-2.2 5-5 5zm0-8c-1.7 0-3 1.3-3 3s1.3 3 3 3 3-1.3 3-3-1.3-3-3-3zm-13-10v43h59v-43h-59zm57 2v24.1l-12.8-12.8c-3-3-7.9-3-11 0l-13.3 13.2-.1-.1c-1.1-1.1-2.5-1.7-4.1-1.7-1.5 0-3 .6-4.1 1.7l-9.6 9.8v-34.2h55zm-55 39v-2l11.1-11.2c1.4-1.4 3.9-1.4 5.3 0l9.7 9.7c-5.2 1.3-9 2.4-9.4 2.5l-3.7 1h-13zm55 0h-34.2c7.1-2 23.2-5.9 33-5.9l1.2-.1v6zm-1.3-7.9c-7.2 0-17.4 2-25.3 3.9l-9.1-9.1 13.3-13.3c2.2-2.2 5.9-2.2 8.1 0l14.3 14.3v4.1l-1.3.1z">'
                                        html = html + '</path>'
                                    html = html + '</svg>'
                                html = html + '</a>'
                            html = html + '</div>'
                            html = html + '<div class="small-text">'
                                html = html + 'Sử dụng nút chọn hình hoặc click vào đây để thêm hình ảnh'
                            html = html + '</div>'
                        html = html + '</div>'
                        ...
                        html = html + '<div class="row mt20 uk-flex uk-flex-middle">'
                            html = html + '<div class="col-lg-2 uk-flex uk-flex-middle">'
                                ...
                                html = html + '<input type="checkbox" '+( (variantData.variant_quantity !== '') ? 'checked' : '' )+' data-target="variantQuantity" class="js-switch">'
                            html = html + '</div>'
                            html = html + '<div class="col-lg-10">'
                                html = html + '<div class="row">'
                                    html = html + '<div class="col-lg-3">'
                                        html = html + '<label for="" class="control-lable">Số lượng</label>'
                                        html = html + '<input type="text" '+( (variantData.variant_quantity == '') ? 'disabled' : '' )+' id="variantQuantity" name="variant_quantity" value="'+variantData.variant_quantity+'" class="form-control '+( (variantData.variant_quantity == '') ? 'disabled' : '' )+' int">'
                                    html = html + '</div>'
                                    html = html + '<div class="col-lg-3">'
                                        html = html + '<label for="" class="control-lable">SKU</label>'
                                        html = html + '<input type="text" name="variant_sku" value="'+variantData.variant_sku+'" class="form-control">'
                                    html = html + '</div>'
                                    html = html + '<div class="col-lg-3">'
                                        html = html + '<label for="" class="control-lable">Giá</label>'
                                        html = html + '<input type="text" name="variant_price" value="'+HT.addCommas(variantData.variant_price)+'" class="form-control int">'
                                    html = html + '</div>'
                                    html = html + '<div class="col-lg-3">'
                                        html = html + '<label for="" class="control-lable">Barcode</label>'
                        ...
                        html = html + '<div class="row mt20 uk-flex uk-flex-middle">'
                            html = html + '<div class="col-lg-2 uk-flex uk-flex-middle">'
                                ...
                                html = html + '<input type="checkbox" '+( (variantData.variant_filename !== '') ? 'checked' : '' )+' class="js-switch" data-target="disabled">'
                            html = html + '</div>'
                            html = html + '<div class="col-lg-10">'
                                html = html + '<div class="row">'
                                    html = html + '<div class="col-lg-6">'
                                        html = html + '<label for="" class="control-lable">Tên File</label>'
                                    ...
                                    html = html + '</div>'
                                    html = html + '<div class="col-lg-6">'
                                        html = html + '<label for="" class="control-lable">Đường dẫn</label>'
    ...
    HT.cancleVariantUpdate = () => {
        $(document).on('click', '.cancleUpdateVariant', function(){
