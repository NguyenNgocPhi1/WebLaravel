(function($){
    ...
    HT.menuRowHtml = (option) => {
        ...
        colums.forEach(col => {
            ...
            .addClass('form-control '+ ((col.name == 'menu[order][]') ? 'int' : ''))
            .attr('name', col.name)
            $col.append($input)
            $row.append($col)
        })
    ...
    HT.renderModelMenu = (object, renderModelMenu) => {
        ...
        html = html + '<div class="m-item">'
            html = html + '<div class="uk-flex uk-flex-middle">'
                html = html + '<input type="checkbox" '+((renderModelMenu.includes(object.canonical)) ? 'checked' : '')+' class="m0 choose-menu" value="'+object.canonical+'" name="" id="'+object.canonical+'">'
